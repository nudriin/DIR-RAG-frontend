...
export default function ConversationPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const answer = data.answer || "Maaf, saya tidak dapat menemukan jawaban.";
          ...
          <div className="space-y-6 pb-4">
            {messages.length === 0 && (
              <div className="h-full flex flex-col items-center justify-center text-center p-8 mt-20 text-muted-foreground animate-in fade-in zoom-in duration-500">
                ...
                <h3 className="text-xl font-semibold mb-2 text-foreground">Humbet AI Assistant</h3>
                <p className="max-w-md text-sm">
                  Silakan ajukan pertanyaan seputar Huma Betang dan budaya Kalimantan Tengah.
                </p>
              </div>
            )}
              ...
              >
                {msg.role === 'assistant' && (
                  <Avatar className="w-8 h-8 border">
                    ...
                    <AvatarImage src="/bot-avatar.png" /> {/* Fallback checks handle missing img */}
                  </Avatar>
                )}
                ...
                <div
                  className={cn(
                    ...
                    msg.role === 'user' 
                      ...
                      : cn("bg-muted/50 border border-border rounded-tl-sm text-foreground", msg.isError && "border-destructive/50 bg-destructive/10 text-destructive")
                  )}
                >
                  {msg.role === 'user' ? (
