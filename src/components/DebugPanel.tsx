    ...
    <div className="space-y-4">
        ...
        <Collapsible title={
          ...
          <div className="space-y-4">
            <div className="flex justify-between text-sm">
              <span className="font-medium text-muted-foreground">Refined Query</span>
              <span className="font-medium text-foreground">{rq_rag.refined_query ?? '—'}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="font-medium text-muted-foreground">Docs Retrieved</span>
              <span className="font-medium text-foreground">{rq_rag.docs_retrieved ?? 0}</span>
            </div>

            {rq_rag.sub_queries && rq_rag.sub_queries.length > 0 && (
              <div className="space-y-2">
                <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Sub Queries</span>
                <div className="flex flex-wrap gap-2">
                  {rq_rag.sub_queries.map((q, i) => (
                    <span key={i} className="rounded-md bg-secondary px-2 py-1 text-xs font-medium text-secondary-foreground text-wrap break-all">
                      {q}
                    </span>
                  ))}
                </div>
            ...
            {rq_rag.source_names && rq_rag.source_names.length > 0 && (
              <div className="space-y-2">
                <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Source Names</span>
                <div className="flex flex-wrap gap-2">
      ...
      {final_status && (
        <Collapsible title={
          ...
          <div className="space-y-4">
            <div className="flex justify-between text-sm">
              <span className="font-medium text-muted-foreground">Stop Reason</span>
              <span className="font-medium text-foreground">{final_status.stop_reason ?? '—'}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="font-medium text-muted-foreground">Is Fallback</span>
              <span className="">
            ...
            {final_status.entropy_history && final_status.entropy_history.length > 0 && (
              <div className="space-y-2 pt-2">
                <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Entropy History</span>
